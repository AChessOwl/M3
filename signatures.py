...
def verify(m, public_key, signed_message):
    ...
    signer          = w3.eth.account.recover_message(message, signature=signed_message.signature)  # Recover signer address
    valid_signature = signer.lower() == public_key.lower()                                          # True if addresses match
